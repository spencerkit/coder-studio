...
export function buildSidecarCommandPageScript(payload: SidecarCommandPayload): string {
  return [
    ...
    '    const clientInstanceId = "desktop-installed-update-acceptance-" + id;',
    ...
    "          return;",
    "        }",
    "        if (message.id === activationId) {",
    "          if (!message.ok) {",
    '            const code = message.error?.code ? String(message.error.code) + ": " : "";',
    '            finish(() => reject(new Error(code + (message.error?.message ?? "Activation claim failed"))));',
    "            return;",
    "          }",
    "          socket.send(",
    "            JSON.stringify({",
