...
const RUNTIME_DIR =
  process.env.CODER_STUDIO_RUNTIME_DIR || path.join(HOME_DIR, '.coder-studio');
...
async function main() {
  ...
  // Give the fire-and-forget HTTP request ~100ms to flush before exiting.
  // Codex runs this script synchronously; exiting immediately would drop the
  // in-flight POST. The request handlers above also call process.exit(0) on
  // error/timeout, so this is just a safety floor for the success path.
