...
async function ensureWorkspaceOpen(page: Page): Promise<void> {
  ...
  if (
    ...
    await expect(page.locator('.agent-draft-launcher, .session-card.agent-pane').first()).toBeVisible({
      timeout: 15000,
    });
    return;
  }
  ...
  await page.locator('.fp-dir').filter({ hasText: /^workspace$/ }).first().dblclick();
  await expect(page.locator('.fp-dir-list .directory-loading')).toHaveCount(0);
  await page.locator('.fp-dir').filter({ hasText: /^coder-studio$/ }).first().click();
