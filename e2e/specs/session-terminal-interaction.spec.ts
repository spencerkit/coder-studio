...
test.describe('session and terminal interaction', () => {
  ...
  test('ST-01: App loads and WebSocket connects', async ({ page }) => {
    ...
    const reconnecting = await page.locator('.connection-banner').filter({ hasText: '重新连接' }).isVisible().catch(() => false);
  ...
  test('ST-03: Terminal panel is accessible', async ({ page }) => {
    ...
    const hasTerminalBtn = await page.locator('button[aria-label*="terminal" i], button:has-text("+")').first().isVisible().catch(() => false);
  ...
  test('ST-04: Agent panes component renders', async ({ page }) => {
    ...
    page.on('pageerror', (err) => {
      if (err.message.includes('xterm') || err.message.includes('jotai') || err.message.includes('terminal')) {
  ...
  test('ST-05: Observer banner (multi-tab fencing)', async ({ browser }) => {
    ...
    await page1.screenshot({ path: SCREENSHOTS_DIR + '/ST-05-tab1-controller.png', fullPage: true });
  ...
  test('ST-06: XtermHost no render errors', async ({ page }) => {
    ...
    const criticalErrors = pageErrors.filter(e =>
      e.toLowerCase().includes('xterm') && !e.toLowerCase().includes('webgl')
    );
