...
import { acquireProcessLock, isDirectExecution } from "@coder-studio/utils";
...
export async function createServer(
  ...
  const shouldWriteRuntimeConfig =
    configOverrides?.writeRuntimeConfig ?? process.env.NODE_ENV === "production";

  ensureStateDir(config);
  const releaseRuntimeLock =
    configuredStateDir === IN_MEMORY_STATE_DIR
      ? () => {}
      : acquireProcessLock({
  ...
  try {
    ...
    eventBus.on<Extract<DomainEvent, { type: "workspace.meta.changed" }>>(
      ...
      (event) => {
        ...
        const runtimeId =
          workspace.targetRuntime === "native" ? "native-default" : "native-default";
    ...
    eventBus.on<Extract<DomainEvent, { type: "session.lifecycle" }>>(
      "session.lifecycle",
      (event) => {
    ...
    if (shouldWriteRuntimeConfig) {
      ...
      writeRuntimeConfig(runtime, configOverrides?.runtimeJsonPath);
    }
    ...
    const stopServer = async () => {
      ...
      try {
        if (shouldWriteRuntimeConfig) {
          deleteRuntimeConfig(configOverrides?.runtimeJsonPath);
        }
      } finally {
        releaseRuntimeLock();
      }
