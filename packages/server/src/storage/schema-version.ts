...
export type SchemaState = "empty" | "current" | "v1" | "v2" | "v3" | "incompatible";
...
export const CURRENT_SCHEMA_VERSION: number = 3;
...
CREATE INDEX idx_custom_providers_updated_at ON custom_providers(updated_at DESC);
...
const V3_SCHEMA_ENTRIES = buildSchemaEntries(V3_SCHEMA_SQL);
...
export function detectSchema(db: Database): SchemaDetection {
  ...
  if (hasExactFingerprint(actualEntries, V3_SCHEMA_ENTRIES)) {
    return {
      state: CURRENT_SCHEMA_VERSION === 3 ? "current" : "v3",
      userVersion,
      mismatch: null,
    };
  }
