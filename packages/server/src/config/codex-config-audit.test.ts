...
import { auditCodexConfigToml, cleanupCodexConfigToml } from "./codex-config-audit.js";
...
describe("codex-config-audit", () => {
  ...
  it("detects [features] codex_hooks = true", () => {
    writeFileSync(
      configPath,
      ['model = "gpt-5"', "", "[features]", "apps = false", "codex_hooks = true"].join("\n")
    );
    const audit = auditCodexConfigToml(configPath);
    expect(audit.findings).toHaveLength(1);
    expect(audit.findings[0]).toMatchObject({
  ...
  it("does NOT flag codex_hooks = false", () => {
    writeFileSync(configPath, ["[features]", "codex_hooks = false"].join("\n"));
    const audit = auditCodexConfigToml(configPath);
    expect(audit.findings).toEqual([]);
  });

  it("does NOT flag codex_hooks = true outside [features]", () => {
    writeFileSync(configPath, ["[other]", "codex_hooks = true"].join("\n"));
    const audit = auditCodexConfigToml(configPath);
    expect(audit.findings).toEqual([]);
  });

  it("does NOT flag notify inside a [section]", () => {
    writeFileSync(configPath, ["[some_section]", 'notify = ["x"]'].join("\n"));
    const audit = auditCodexConfigToml(configPath);
    expect(audit.findings).toEqual([]);
  });
  ...
  it("detects both notify and codex_hooks in one pass", () => {
    writeFileSync(
      configPath,
      ['model = "gpt-5"', 'notify = ["agent-notify"]', "", "[features]", "codex_hooks = true"].join(
        "\n"
      )
    );
    const audit = auditCodexConfigToml(configPath);
