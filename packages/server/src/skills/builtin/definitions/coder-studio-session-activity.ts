...
const CONTENT = [
  ...
  "",
  "```bash",
  `node "${AUTOMATION_CMD_ABSOLUTE_PATH_TOKEN}" session.activity.record --kind plan --phase update --title "Refine implementation plan" --summary "Refined the implementation plan after reviewing the registry tests." --json`,
  `node "${AUTOMATION_CMD_ABSOLUTE_PATH_TOKEN}" session.activity.record --kind command --phase finish --title "Run targeted vitest" --summary "Targeted Vitest run failed because the session activity skill definition was missing." --status warning --command 'pnpm exec vitest run packages/server/src/__tests__/skills/builtin-registry.test.ts --testNamePattern "session activity skill|metadata descriptions"' --payload-json '{"exitCode":1}' --json`,
  `node "${AUTOMATION_CMD_ABSOLUTE_PATH_TOKEN}" session.activity.record --kind edit --phase finish --title "Register built-in skill" --files '["packages/server/src/skills/builtin/definitions/coder-studio-session-activity.ts","packages/server/src/skills/builtin/definitions/index.ts","packages/server/src/skills/builtin/registry.ts"]' --summary "Added the built-in session activity skill definition and registered it." --json`,
  "```",
  "",
  "## Review Activity",
  "",
