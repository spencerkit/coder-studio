import { CODER_STUDIO_CANVAS_SKILL } from "./coder-studio-canvas.js";
import { CODER_STUDIO_MEMORY_SKILL } from "./coder-studio-memory.js";
import { CODER_STUDIO_OPEN_SKILL } from "./coder-studio-open.js";
import { CODER_STUDIO_SESSION_ACTIVITY_SKILL } from "./coder-studio-session-activity.js";

export { CODER_STUDIO_CANVAS_SKILL } from "./coder-studio-canvas.js";
export { CODER_STUDIO_MEMORY_SKILL } from "./coder-studio-memory.js";
export { CODER_STUDIO_OPEN_SKILL } from "./coder-studio-open.js";
export { CODER_STUDIO_SESSION_ACTIVITY_SKILL } from "./coder-studio-session-activity.js";
export type { BuiltinSkillDefinition } from "./types.js";

export const BUILTIN_SKILLS = [
  CODER_STUDIO_OPEN_SKILL,
  CODER_STUDIO_MEMORY_SKILL,
  CODER_STUDIO_CANVAS_SKILL,
  CODER_STUDIO_SESSION_ACTIVITY_SKILL,
];
