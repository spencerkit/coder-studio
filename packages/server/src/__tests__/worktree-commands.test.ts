...
describe('Worktree Commands', () => {
  ...
  beforeEach(async () => {
    repoDir = join(tmpdir(), `worktree-command-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    otherRepoDir = join(tmpdir(), `worktree-command-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  it.each(['worktree.status', 'worktree.diff', 'worktree.tree'] as const)(
    'rejects %s for a git repo outside the workspace worktree set',
    async (op) => {
      const result = await dispatch(
        {
          kind: 'command',
