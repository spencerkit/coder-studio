...
describe("runGitPush", () => {
  ...
  it("persists successful HTTP credentials through the configured credential helper", async () => {
    ...
    await writeFile(
      helperScript,
      ["#!/bin/sh", `cat > ${JSON.stringify(helperLog)}`, "exit 0", ""].join("\n"),
      { mode: 0o700 }
    );
    await mkdir(wrapperDir, { recursive: true });
    await writeFile(
  ...
  it("forces the entered username for the authenticated retry", async () => {
    ...
    await execFileAsync(
      "git",
      ["remote", "add", "origin", "https://configured@example.com/openai/demo.git"],
      { cwd: testDir }
    );
    const originalPath = process.env.PATH ?? "";
    vi.stubEnv("PATH", `${wrapperDir}:${originalPath}`);
  ...
  it("persists credentials when a URL-scoped credential helper is configured", async () => {
    ...
    await execFileAsync("git", ["config", "credential.https://example.com.helper", helperScript], {
      cwd: testDir,
    });
    const originalPath = process.env.PATH ?? "";
    vi.stubEnv("PATH", `${wrapperDir}:${originalPath}`);
