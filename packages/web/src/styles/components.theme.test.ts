...
describe("components.css theme-sensitive surfaces", () => {
  ...
  it("keeps the workspace launch modal theme-aware", () => {
    ...
    expect(modal).toContain("var(--component-mix-surface-panel-82pct-surface-page-18pct)");
  ...
  it("keeps workspace chrome on tokens instead of hardcoded dark fills", () => {
    ...
    const workspaceSidebarContent = getLastRuleBlock(".workspace-sidebar-panel__content");
    const workspaceSidebarView = getLastRuleBlock(".workspace-sidebar-view");
    ...
    expect(sessionCard).not.toContain("border: 1px solid var(--border-default)");
    ...
    expect(resolvingConsoleStatus).toContain(
      "var(--component-mix-state-success-border-32pct-transparent)"
    );
    expect(resolvingSkeleton).toContain(
      "background: var(--component-mix-surface-hover-74pct-surface-panel-26pct)"
    );
    ...
    expect(activityBar).toContain("border-right: 1px solid var(--border-default)");
  ...
  it("keeps auth and welcome shells on flat page surfaces", () => {
    ...
    expect(authScreen).not.toContain("var(--component-rgba-17-24-31-0-96)");
  ...
  it("styles the welcome page as a step-first workflow with compact support content", () => {
    ...
    expect(welcomeCard).toContain("width: min(100%, 1180px)");
    expect(welcomeCard).toContain("max-width: 1180px");
    expect(welcomeLayout).toContain("display: grid");
    expect(welcomeLayout).toContain("grid-template-columns: minmax(280px, 0.8fr) minmax(0, 1.3fr)");
    ...
    expect(stepDetail).toContain("max-width: 40ch");
    expect(stepDetail).toContain("color: var(--text-secondary)");
    expect(welcomeTitle).not.toContain("max-width");
    expect(welcomeBody).not.toContain("max-width");
  });
  ...
  it("routes settings and workspace shared surfaces through appearance-aware background tokens", () => {
    ...
    const workspaceStatusBar = getLastRuleBlock(".workspace-status-bar");
    const workspaceSidebarContent = getLastRuleBlock(".workspace-sidebar-panel__content");
    const workspaceSidebarView = getLastRuleBlock(".workspace-sidebar-view");
    const workspaceSidebarBody = getLastRuleBlock(".workspace-sidebar-panel__body");
    const sessionCard = getLastRuleBlock(".session-card");
    ...
    const activeSessionHeader = getRuleBlocksFrom(
      ...
      .join("\n");
    const sessionTerminal = getLastRuleBlock(".session-terminal");
    const workspaceEmptyInner = getLastRuleBlock(".workspace-empty-inner");
    const workspaceResolvingCard = getLastRuleBlock(".workspace-resolving-card");
    ...
    expect(workspaceSidebarPanel).toContain(
      "background: color-mix(in srgb, var(--surface-page) 84%, var(--surface-panel) 16%)"
    );
    expect(workspaceSidebarPanel).toContain("backdrop-filter: none");
  ...
  it("keeps workspace editor and diff surfaces theme-aware", () => {
    ...
    expect(codeModeToggleRules).toContain(
      "background: var(--component-mix-surface-hover-72pct-surface-panel-28pct)"
    );
    ...
    expect(
      sessionDotBlocks.some(
        (block) =>
          ...
          block.includes(
            "var(--component-mix-status-dot-current-color-var-text-tertiary-12pct-transparent)"
          )
      )
    ).toBe(true);
  ...
  it("keeps panel headers on the approved dense container chrome contract", () => {
    const panelHeader =
      [...getRuleBlocksFrom(stylesheet, ".panel-header")]
        .reverse()
        .find((block) => block.includes("border-bottom: 1px solid var(--border-default)")) ?? "";
    ...
    expect(panelHeader).toContain("border-bottom: 1px solid var(--border-default)");
  ...
  it("keeps supervisor dialog body styling flat and dense", () => {
    ...
    expect(intro).toContain(
      "border: 1px solid var(--component-mix-border-default-90pct-transparent)"
    );
    expect(intro).toContain("background: var(--component-mix-surface-panel-88pct-surface-hover)");
  ...
  it("keeps supervisor strip, history badges, and restore cards on shared state tokens", () => {
    ...
    expect(manualTrigger).toContain(
      "background: var(--component-mix-icon-file-data-16pct-surface-panel)"
    );
  ...
  it("keeps mobile select row-side actions lightweight and token-driven", () => {
    ...
    expect(rowSelected).toContain("var(--component-mix-status-info-fg-10pct-surface-panel)");
    expect(plainSelected).toContain("var(--component-mix-status-info-fg-10pct-surface-panel)");
    expect(commandSelected).toContain("var(--component-mix-status-info-fg-12pct-surface-panel)");
  ...
  it("keeps settings navigation aligned with desktop editor chrome on both desktop and mobile", () => {
    ...
    expect(settingsNavItemActive).toContain(
      "border-color: var(--component-mix-status-info-fg-42pct-border-default-58pct)"
    );
    expect(settingsNavItemActive).toContain(
      "box-shadow: inset 0 0 0 1px var(--component-mix-status-info-fg-18pct-transparent)"
    );
  ...
  it("keeps the mobile workspace launch action docked in a compact editor-style footer rail", () => {
    ...
    expect(launchActionRail).toContain(
      "background: var(--component-mix-surface-panel-96pct-surface-page)"
    );
  ...
  it("keeps folder picker selection affordances on shared state tokens", () => {
    ...
    expect(directoryHover).toContain(
      "background: var(--component-mix-state-selected-bg-72pct-transparent)"
    );
    ...
    expect(directoryActionHover).toContain(
      "background: var(--component-mix-state-info-bg-78pct-surface-panel-22pct)"
    );
    ...
    expect(directoryActionFocus).toContain(
      "var(--component-mix-state-focus-ring-color-24pct-transparent)"
    );
  });
  ...
  it("keeps the mobile workspace home screen aligned to settings chrome and editor-pane empty states", () => {
    ...
    expect(bottomStack).toContain(
      "border-top: 1px solid var(--component-mix-border-default-84pct-transparent)"
    );
  ...
  it("keeps mobile container surfaces on shared radius tokens instead of bespoke rounded-card values", () => {
    ...
    const workspaceSidebarPanel = getLastRuleBlock(".workspace-sidebar-panel");
    const mobileFilesSurfaceBlocks = getRuleBlocksFrom(
      stylesheet,
      ".mobile-sheet--files .file-tree-shell--mobile"
    );
    const mobileFilesGitSurfaceBlocks = getRuleBlocksFrom(
      stylesheet,
    ...
    const mobileFilesSegmented = getLastRuleBlock(
      ".mobile-sheet--files .mobile-files-sheet__segmented"
    );
    const mobileFilesSegment = getLastRuleBlock(
      ".mobile-sheet--files .mobile-files-sheet__segment"
    );
    const mobileFilesSegmentIcon = getLastRuleBlock(".mobile-files-sheet__segment-icon");
    const mobileFilesSegmentActive = getLastRuleBlock(
      ".mobile-sheet--files .mobile-files-sheet__segment.active"
    );
    const mobileFilesSegmentIndicator = getLastRuleBlock(
      ".mobile-sheet--files .mobile-files-sheet__segment.active::after"
    );
    ...
    const mobileFileRowBlocks = getRuleBlocksFrom(
      stylesheet,
    ...
    expect(mobileFilesContent).toContain("padding: 0 16px 16px");
    expect(hasRuleBlock(".mobile-supervisor-sheet__root")).toBe(false);
    expect(hasRuleBlock(".mobile-supervisor-sheet__detail-header")).toBe(false);
    expect(mobileFilesSegmented).toContain("padding: 16px 16px 14px");
    ...
    expect(mobileFilesSegment).toContain("border-radius: 6px");
    expect(mobileFilesSegmentIcon).toContain("display: block");
    expect(mobileFilesSegmentActive).toContain(
      "background: var(--component-mix-status-info-fg-14pct-transparent)"
    );
    expect(mobileFilesSegmentActive).toContain(
      "border-color: var(--component-mix-border-default-72pct-transparent)"
    );
    ...
    expect(mobileFilesSurfaceBlocks.some((block) => block.includes("linear-gradient("))).toBe(
      false
    );
    expect(
      mobileFilesGitSurfaceBlocks.some(
        (block) =>
          block.includes("border: none") &&
          block.includes("background: transparent") &&
          block.includes("box-shadow: none")
    ...
    expect(
      mobileFileRowBlocks.some(
        (block) => block.includes("min-height: 40px") && block.includes("border-radius: 4px")
      )
    ).toBe(true);
  ...
  it("stacks mobile welcome and auth shells vertically so cards size to content", () => {
    ...
    const mobileWelcomeFeatures = getLastRuleBlock(".welcome-card--mobile .welcome-card__features");
    ...
    const mobileWelcomeTitle = getLastRuleBlock(".welcome-card--mobile .welcome-title");
    const mobileWelcomeBody = getLastRuleBlock(".welcome-card--mobile .welcome-body");
    ...
    expect(mobileWelcomeFeatures).toContain("width: 100%");
    expect(mobileWelcomeFeatures).toContain("align-self: stretch");
    expect(mobileWelcomeSteps).toContain("grid-template-columns: 1fr");
    expect(mobileWelcomeTitle).not.toContain("max-width");
    expect(mobileWelcomeBody).not.toContain("max-width");
  ...
  it("keeps settings content groups and provider controls aligned with editor configuration panels", () => {
    ...
    expect(settingsInfoRowBase).toContain("border-bottom: 1px solid var(--border-default)");
    ...
    expect(providerMobileEntryBase).toContain("border-top: 1px solid var(--border-default)");
    ...
    expect(commandPreview).toContain(
      "background: var(--component-mix-surface-input-82pct-surface-panel-18pct)"
    );
    expect(settingsFooter).toContain("background: var(--surface-panel)");
    expect(settingsFooter).toContain("border-top: 1px solid var(--border-default)");
  ...
  it("keeps background-material settings on layered surfaces with a responsive material grid", () => {
    ...
    expect(appearancePanel).toContain(
      "background: var(--component-mix-surface-input-72pct-surface-panel-28pct)"
    );
    ...
    expect(overridePanel).toContain(
      "background: var(--component-mix-surface-panel-74pct-transparent)"
    );
  ...
  it("keeps shortcut settings rows aligned with editor-pane list treatment instead of standalone cards", () => {
    ...
    expect(categoryTabActive).not.toContain("var(--component-rgba-108-182-255-0-2)");
    expect(listBase).toContain("gap: 0");
    expect(listBase).toContain("border-top: 1px solid var(--border-default)");
    ...
    expect(itemBase).toContain("border-bottom: 1px solid var(--border-default)");
    ...
    expect(customItem).toContain(
      "background: var(--component-mix-surface-active-78pct-status-warning-fg-22pct)"
    );
    ...
    expect(footerBase).toContain("border-top: 1px solid var(--border-default)");
  ...
  it("keeps provider config editing closer to embedded editor panes than modal-like cards", () => {
    ...
    expect(configCardActionsBase).toContain("border-top: 1px solid var(--border-default)");
    ...
    expect(configErrorBase).toContain(
      "background: var(--component-mix-status-danger-fg-10pct-surface-panel)"
    );
    expect(configErrorBase).toContain("border-top: 1px solid var(--border-default)");
  });
  ...
  it("keeps provider base controls aligned with inspector-style config rows on desktop and mobile", () => {
    ...
    expect(argsInput).toContain(
      "background: var(--component-mix-surface-input-82pct-surface-panel-18pct)"
    );
    expect(argsInput).toContain("border-color: var(--border-default)");
  ...
  it("keeps the desktop file tree search and selected row aligned with the polished panel chrome", () => {
    ...
    expect(row).toContain("padding-top: 0");
    expect(row).toContain("padding-bottom: 0");
    expect(row).toContain("border-radius: var(--radius-sm)");
    ...
    expect(rowSelected).toContain("--workspace-sidebar-selected-border");
    expect(rowSelected).toContain("--workspace-sidebar-selected-bg");
  ...
  it("keeps workspace search and quick open on compact editor-search chrome", () => {
    ...
    expect(sidebarRowSelectedHover).toContain("--workspace-sidebar-selected-hover-bg");
    ...
    expect(searchGroupAdjacent).toContain("margin-top: var(--gap-tight)");
    expect(searchGroupHeader).toContain("grid-template-columns: 18px minmax(0, 1fr) auto");
    ...
    expect(searchGroupHeader).toContain("box-shadow: none");
    expect(searchGroupPath).toContain("display: none");
    expect(searchMatches).toContain("padding: 2px 0 var(--sp-1)");
    expect(searchMatch).toContain("grid-template-columns: 28px minmax(0, 1fr) auto");
    expect(searchMatch).toContain("min-height: 32px");
    expect(searchMatch).toContain("border-radius: var(--radius-sm)");
    expect(searchMatch).toContain("border: 1px solid transparent");
    expect(searchMatchActive).toContain("--workspace-sidebar-selected-border");
    expect(searchMatchActive).toContain("--workspace-sidebar-selected-bg");
    expect(searchMatchActiveHover).toContain("--workspace-sidebar-selected-border");
    expect(searchMatchActiveHover).toContain("--workspace-sidebar-selected-hover-bg");
  ...
  it("keeps the desktop git panel and command palette on tighter tool-surface chrome", () => {
    ...
    expect(gitCommitBlock).toContain("border-bottom: none");
    ...
    expect(gitHistoryRowCurrent).toContain("--workspace-sidebar-selected-border");
    expect(gitHistoryRowCurrent).toContain("--workspace-sidebar-selected-bg");
    expect(commandPalette).toContain("max-width: var(--desktop-modal-max-width-md)");
  ...
  it("keeps running session header emphasis theme-safe and motion-aware", () => {
    ...
    expect(runningBadge).toContain(
      "border: 1px solid var(--component-mix-current-color-22pct-transparent)"
    );
    expect(runningBadge).toContain(
      "background: var(--component-mix-current-color-12pct-transparent)"
    );
  ...
  it("keeps the mobile terminal keybar flow-positioned and token-driven", () => {
    ...
    expect(ctrlLocked).toContain("var(--component-mix-status-info-fg-42pct-border-default)");
    expect(ctrlLocked).toContain("var(--component-mix-status-info-fg-12pct-surface-panel)");
    expect(shiftArmed).toContain("var(--component-mix-status-info-fg-28pct-border-default)");
    expect(shiftArmed).toContain("var(--component-mix-status-info-fg-8pct-surface-panel)");
  });
