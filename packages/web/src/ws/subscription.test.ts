...
describe("subscription topic helpers", () => {
  ...
  it("builds terminal topics through Topics-compatible helpers", () => {
    ...
    expect(terminalTopic("ws-1", "term-1", "output")).toBe(Topics.terminalOutput("ws-1", "term-1"));
    expect(terminalTopic("ws-1", "term-1", "exit")).toBe(Topics.terminalExit("ws-1", "term-1"));
  });

  it("builds session topics through Topics-compatible helpers", () => {
    expect(sessionTopic("ws-1", "sess-1", "state")).toBe(Topics.sessionState("ws-1", "sess-1"));
