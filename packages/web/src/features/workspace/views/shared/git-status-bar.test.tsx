...
describe('GitStatusBar', () => {
  ...
  it('shows unsupported auth guidance when the remote cannot prompt in-app', async () => {
    ...
    expect(await screen.findByText('origin (github.com) 认证失败。请先配置 SSH key 或 credential helper，再重试。')).toBeInTheDocument();
    expect(screen.getByText('当前远端不支持应用内认证。请先配置 SSH key 或 credential helper，再重试。')).toBeInTheDocument();
  ...
  it('keeps the typed username after an invalid credential retry', async () => {
    ...
    expect(await screen.findByText('Credentials for origin (github.com) were rejected. Enter a valid username and password or personal access token to continue.')).toBeInTheDocument();
    expect(screen.getByLabelText('Username')).toHaveValue('alice');
