...
import { openDatabase } from '../../server/src/storage/index.js';
import { listAuthBlocks, clearAuthBlockByIp } from './auth-control.js';
...
describe('auth-control', () => {
  ...
  it('lists and clears active auth blocks from the configured sqlite database', async () => {
    ...
    try {
      const failures = verificationDb.prepare('SELECT ip, failed_at FROM auth_login_failures WHERE ip = ?').all('198.51.100.24');
