...
describe('pm2-control', () => {
  ...
  beforeEach(() => {
    ...
    deleteProcess.mockImplementation((_name: string, callback: (error: Error | null) => void) => callback(null));
    describeProcess.mockImplementation((_name: string, callback: (error: Error | null, result: unknown[]) => void) =>
      callback(null, [])
    );
  });
  ...
  it('waits for the previous PM2 app to disappear before starting a replacement', async () => {
    describeProcess
      ...
      .mockImplementationOnce(
        (_name: string, callback: (error: Error | null, result: unknown[]) => void) => callback(null, [])
      );
